...
bool drakon::Renderer::createCommandObjects() {
    HRESULT hr =
        this->d3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&this->commandAllocator));
    if (FAILED(hr)) {
...
bool drakon::Renderer::createSwapChain() {
    ...
    HRESULT                                 hr = this->dxgiFactory->CreateSwapChainForHwnd(
        this->commandQueue.Get(), this->windowHandle, &desc, nullptr, nullptr, &swapChain1);
...
bool drakon::Renderer::render(std::vector<Renderable*> renderables) {
    ...
    */
    ...
    D3D12_VIEWPORT viewport = {
        0.0f, 0.0f, static_cast<float>(this->windowWidth), static_cast<float>(this->windowHeight), 0.0f, 1.0f};
