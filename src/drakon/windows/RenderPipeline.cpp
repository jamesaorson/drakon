...
bool drakon::RenderPipeline::initialize(ID3D12GraphicsCommandList&          commandList,
                                        const drakon::RenderPipelineConfig& config,
                                        drakon::RenderPipelineState&        outState) {
    ...
    if (FAILED(hr)) {
        if (errorBlob) {
            std::cerr << "Root signature error: " << static_cast<const char*>(errorBlob->GetBufferPointer())
                      << std::endl;
        }
        return false;
    }
    ...
    hr = D3DCompile(config.shaderSource,
    ...
    if (FAILED(hr)) {
        if (errorBlob) {
            std::cerr << "Vertex shader error: " << static_cast<const char*>(errorBlob->GetBufferPointer())
                      << std::endl;
        }
        return false;
    }
    ...
    if (FAILED(hr)) {
        if (errorBlob) {
            std::cerr << "Pixel shader error: " << static_cast<const char*>(errorBlob->GetBufferPointer())
                      << std::endl;
        }
        return false;
    }
